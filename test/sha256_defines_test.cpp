...
TEST(sha256, sha256_functions)
{
	...
	EXPECT_EQ(BSIG0(0x000007FF), 0x00FE7FDF); //      2047 ERROR
	...
	EXPECT_EQ(BSIG1(0x0000007F), 0xF3E03F81); //       127
	EXPECT_EQ(BSIG1(0x000007FF), 0x03E3FF9F); //      2047
	EXPECT_EQ(BSIG1(0x0000184F), 0x35EC27E2); //      6223
	EXPECT_EQ(BSIG1(0x007FFFFF), 0x3C1E0F80); //   8388607
	EXPECT_EQ(BSIG1(0x01343CAC), 0xBF9AA075); //  20200620
	EXPECT_EQ(BSIG1(0x0202E912), 0x6B3CC2F8); //  33745170
	EXPECT_EQ(BSIG1(0x1FFFFFFF), 0xFC63FF8F); // 536870911
	EXPECT_EQ(BSIG1(0x26DE0C36), 0x3159B8E2); // 652086326

	//Test cases for SSIG0
	EXPECT_EQ(SSIG0(0x00000000), 0x00000000);
	EXPECT_EQ(SSIG0(0xFFFFFFFF), 0xFFFFFFFF); // ERROR

	EXPECT_EQ(SSIG0(0x0000007F), 0xFE1FC00F); //       127
	EXPECT_EQ(SSIG0(0x000007FF), 0xFFFFC0F0); //      2047
	EXPECT_EQ(SSIG0(0x0000184F), 0x9813C339); //      6223
	EXPECT_EQ(SSIG0(0x007FFFFF), 0x01F0C01F); //   8388607
	EXPECT_EQ(SSIG0(0x01343CAC), 0x570FEFA1); //  20200620
	EXPECT_EQ(SSIG0(0x0202E912), 0x9E00D870); //  33745170
	EXPECT_EQ(SSIG0(0x1FFFFFFF), 0x023FC7FF); // 536870911
	EXPECT_EQ(SSIG0(0x26DE0C36), 0xEB9BF429); // 652086326

	//Test cases for SSIG1
	EXPECT_EQ(SSIG1(0x00000000), 0x00000000);
	EXPECT_EQ(SSIG1(0xFFFFFFFF), 0xFFFFFFFF); // ERROR

	EXPECT_EQ(SSIG1(0x0000007F), 0x00306000); //       127
	EXPECT_EQ(SSIG1(0x000007FF), 0x03006001); //      2047
	EXPECT_EQ(SSIG1(0x0000184F), 0x0F2E6006); //      6223
	EXPECT_EQ(SSIG1(0x007FFFFF), 0x00007FCF); //   8388607
	EXPECT_EQ(SSIG1(0x01343CAC), 0x99C3CDB3); //  20200620
	EXPECT_EQ(SSIG1(0x0202E912), 0x29ABC1FB); //  33745170
	EXPECT_EQ(SSIG1(0x1FFFFFFF), 0x000793FF); // 536870911
	EXPECT_EQ(SSIG1(0x26DE0C36), 0xC7946037); // 652086326
